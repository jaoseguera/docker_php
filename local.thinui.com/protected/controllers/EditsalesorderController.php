<?php
class EditsalesorderController extends Controller
{
    /**
    ...
    **/
    public function actionEditsalesorder()
    {
	...
	public function actionStringpdf()
    {
		...
		else
		{
			...
			//echo $str;
			$_SESSION['pdfname'] = 'Sales_Order_Confirmation_'.ltrim($order, '0');
			$_SESSION['pdfstr']  = $str;
		}
	}
	...
	public function actionSave_sales_order()
    {
        if(Yii::app()->user->hasState("login"))
        {            
            global $rfc, $fce;
            ...
            $b = new Bapi();
            $b->bapiCall($bapiName);
            Yii::app()->controller->renderPartial('save_sales_order',array('model'=>$model,'fce'=>$fce));
        }
        else{
            $this->redirect(array('login/'));
        }
