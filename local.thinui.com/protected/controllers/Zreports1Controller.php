<?php
class Zreports1Controller extends Controller
{
    /**
    ...
    **/
    public function actionZreports1()
    {
        if(Yii::app()->user->hasState("login"))
        {
            $model = new Zreports1Form;
            if(isset($_POST['key']))
            {
                global $rfc, $fce;
	...
	public function actionCustomforms()
	{
		if(Yii::app()->user->hasState("login"))
		{
			...
			if(isset($_REQUEST['subkey']))
			{
				...
				for ($i=0;$i<$rowsag;$i++)
					$SalesOrder[]= $res["ET_REPORT_TEXT"][$i];
				
				echo 'ZP$@$'.json_encode($SalesOrder);
			}
			else
			{
				$bapiName=Controller::Bapiname($_REQUEST['key']);
