<?php
class Create_sales_quotation_soldController extends Controller
{
    /**
    ...
    **/
    public function actionCreate_sales_quotation_sold()
    {
	 ...
	 public function actionQuotaitonemail()
	{
		...
		$mail->IsSMTP();
                ...
                if(isset($doc->mailserver) && ($doc->mailserver->host!=''))
                {
                        ...
                        {
                              echo basename($file).' '._NOTCONFIGFOLDER;
                              exit;
                        }
		...
		$mail->Subject    = _QUOTATIONMAILSUBJECT;
		$filename ='';
		$file='';
		
		global $rfc, $fce;
		$order=$_REQUEST['q_no'];;
		$bapiName=Controller::Bapiname('editsalesquotation_doc');
		...
		$msg= $res["ET_MESSAGES"][0];
			...
			//echo $str;
			$filename = _QUOTATIONPDFMAIL.ltrim($order, '0').'.pdf';
			//$file= $str;
			$_SESSION['pdfname']=_QUOTATIONPDFMAIL.ltrim($order, '0');
			$_SESSION['pdfstr']=$str;
			$file=$str;
		...
		$mail->IsHTML(false);
        //$_REQUEST['name'];
		$mail->MsgHTML(_DOCUMENTBEINGEMAILED.' '.$user_name.'  '._PLEASEFINDATTACHMENT); //Text Body
		}
