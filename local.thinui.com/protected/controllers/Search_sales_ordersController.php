<?php
class Search_sales_ordersController extends Controller
{
    /**
 ...
 public function actionTabledata() {
	     if (Yii::app()->user->hasState("login")) {
            global $rfc, $fce;
			$model = new Search_sales_ordersForm;
    ...
    public function actionSearch_sales_orders()
    {
        if(Yii::app()->user->hasState("login"))
        {            
            $model = new CommonForm;
			$models = new Search_sales_ordersForm;
			$userid = Yii::app()->user->getState('user_id');
            $client = Controller::userDbconnection();
			$doc = $client->getDoc($userid);
			$screen = CommonController::setScreen();
