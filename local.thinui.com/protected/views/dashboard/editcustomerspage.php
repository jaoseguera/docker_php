...
function changesave()
{
    ...
    $('#edit').replaceWith('<button class="btn btn-primary spanbt span2" type="submit" id="save_id" style="min-width:70px">'+edit+'</button>');
    $('#can_id').show();
}
function changeedit()
{
			...
			<div class="utopia-widget-content">
				<form id="validation1" action="javascript:edit_customer()" class="form-horizontal editcus" >
					...
					<fieldset class="marg"> 
						...
						<div class="control-group">
						<label class="control-label cutz" for="date" alt='City'><?=Controller::customize_label(CITY);?><span>*</span>:</label>
						<div class="controls">
						...
						<fieldset >  
							...
							<div class="control-group">
								...
								<input type="text" name='PI_SALESORG' id='SALES_ORG' class="input-fluid validate[required,custom[number]] getval radius read"/>
                                    ...
                                    <span class='minw' onclick="lookup('<?=Controller::customize_label(_SALESORGANIZATION);?>', 'SALES_ORG', 'sales_org')" >&nbsp;</span>
								</div>
							</div>
							<div class="control-group">
								<label class="control-label cutz" for="input01" alt='Distribution. Channel'><?=Controller::customize_label(_DISTRIBUTIONCHANNEL);?><span>*</span>:</label>
								<div class="controls">
								<input type="text" name='PI_DISTR_CHAN' id='DISTR_CHAN' class="input-fluid validate[required,custom[dis]] getval radius read"/>
                                    ...
                                    <span class='minw' onclick="lookup('<?=Controller::customize_label(_DISTRIBUTIONCHANNEL);?>', 'DISTR_CHAN', 'dist_chan')" >&nbsp;</span>
								</div>
							</div>
							<div class="control-group">
								...
								<input type="text" name='PI_DIVISION' id='DIVISION' class="input-fluid validate[required,custom[divi]] getval radius read"/>
                                    ...
                                    <span class='minw' onclick="lookup('<?=Controller::customize_label(_DIVISION);?>', 'DIVISION', 'division')" >&nbsp;</span>
								</div>
							</div>
					</fieldset></div>
					<div class="span6 utopia-form-freeSpace bbr" style="padding-bottom:10px;"id="bottomform">
						<div >
							<table><tr><td><button class="btn  spanbt span2 back_b" type="button" id='edit' style="min-width:70px;" onclick="return changesave()"><?=_EDIT?></button></td><td>&nbsp;</td><td>
