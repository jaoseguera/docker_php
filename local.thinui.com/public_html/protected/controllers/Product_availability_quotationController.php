<?php
class Product_availability_quotationController extends Controller
{
    /**
    ...
    **/
    public function actionProduct_availability_quotation()
    {
        if(Yii::app()->user->hasState("login"))
        {
            $model = new Product_availability_quotationForm;
            if(isset($_POST['key']))
            {
                global $rfc, $fce;            
    ...
    public function actionProduct_avl()
    {
        if(Yii::app()->user->hasState("login"))
        {
            ...
            Yii::app()->controller->renderPartial('/common/product_avl_quotation',array('model'=>$model,'SalesOrder'=>$SalesOrder,'MATERIAL'=>$MATERIAL));
        }
        else{
            $this->redirect(array('login/'));
        }
