...
@media all and (min-width: 751px) {
	#add_row_table .table th:nth-child(-n+6), #add_row_table .table td:nth-child(-n+6) {
        display: table-cell ;
    .check { display:none !important; }
	}
}
               ...
               <div class="span5 utopia-form-freeSpace">
                  ...
                  <fieldset>
                     <div class="control-group" >
                        ...
                        <label class="control-label cutz" for="input01" alt='Sale Order Number' ><?php echo Controller::customize_label('Sale Order Number'); ?><span> *</span>:&nbsp;</label>
                        <input type="hidden" name='page' value="bapi">
                        <input type="hidden" name="url" value="editsalesorder"/>
                        <input type="hidden" name="key" value="editsalesorder"/>
                        ...
                        <div class="controls">
                           <input style='min-width:170px;' id='SALES_DOCUMENT' class="input-fluid validate[required]" type="text" name='I_VBELN' value="<?php echo $I_VBELN_TXT; ?>"><span  class='minw' onclick="lookup('Sales Document', 'SALES_DOCUMENT', 'sales_document')" >&nbsp;</span>
                        </div>
...
<?php if (isset($_REQUEST['I_VBELN']) && $ORDER_ITEMS_OUT != NULL) {
   ...
   <input type="hidden" name="bapiName" id="bapiName" value="BAPI_SALESORDER_CHANGE"/>
   ...
   <div id="form_edit_values" class="utopia-widget-content myspace inval35 spaceing row-fluid" style="margin-top:11px;">
               ...
               <div class="control-group">
                  ...
                  <div class="controls">
                     <input alt="Sales Organization" type="text" name='SALES_ORG' id='SALES_ORG' class="input-fluid validate[required,custom[salesorder]] getval radius" value='<?php echo $sales_org; ?>' onKeyUp="jspt('SALES_ORG',this.value,event)" autocomplete="off"/>
                      ...
                      <div class='minws' onclick="lookup('Sales Organization', 'SALES_ORG', 'sales_org')" >&nbsp;</div>
         ...
         <div class="span5 utopia-form-freeSpace">
            <fieldset>
               ...
               <div class="control-group" >
                  <label class="control-label cutz" for="input01" style='width:150px;' alt='Requested Delivery Date'><?php echo Controller::customize_label('Requested Delivery Date'); ?><span> *</span>:</label>
                  <div class="controls">
                     <input alt="Requested Delivery Date" type="text" id="datepicker" name='Delivery'  class="input-fluid getval radius" value="<?php echo $date_formate; ?>"/>
                  </div></div>
               <div class="control-group" >
                  <div class="controls">
                     <input type="button" name="copy_form_data" id="copy_form_data" value="Copy Sales Order" class="btn" />
                  </div></div>
            </fieldset>
         </div>
							...
							<tbody>
								<tr onClick="select_row('ids_0')" class="ids_0 nudf" >
									<td><input class="chkbox check" type="checkbox" name="checkbox[]"
                                        ...
                                        <input  type="text" name='item[]' <?php if(!isset($customerNo))
									...
									<td><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/icons/info.png" style="margin-left: -10%; cursor: pointer;" />
										<input type="text"  id='MATERIAL' name='material[]' class="input-fluid validate[required] getval radiu" title="MATERIAL" alt="MULTI" style="width:55% !important;" onKeyUp="jspt('MATERIAL',this.value,event)" autocomplete="off" value='<?php echo $meterial;?>' onchange="jspt_new('MATERIAL',this.value,event)"/>
									...
									<td>
										<input type="text" style="width:55% !important;" id='TARGET_QU' class="input-fluid validate[required] getval radiu" name='su[]' title="su" alt="MULTI" onKeyUp="jspt('TARGET_QU',this.value,event)" autocomplete="off" value='<?php echo $su;?>'/>
                                        <!--<span  class='minw9'  onclick="tipup('BUS2032','CREATEFROMDAT2','ORDERITEMSIN','TARGET_QU','SU','TARGET_QU','0')" >&nbsp;</span>-->
                                        <div  class='minws1' id="table_lookup" onclick="lookup('SU', 'TARGET_QU', 'uom')" >&nbsp;</div>
   ...
   <div class="controls" style="margin-left:0px;">
                     ...
                     <input type="button" name="save_salesorder" id="save_salesorder" value="Save" onClick="save_order()" style="display:none;" class="btn btn-primary iphone_sales_disp" />
...
<?php
	...
	{
		?>
			<div class="tab-content tab-con">Sales order doesn't exist.</div>
		<?php
	}
	?>
...
function save_order()
{
    ...
    $.ajax({
        ...
        url: 'editsalesorder/save_sales_order',			
        success: function(response) 
        {
			...
			if(type != "E")
			{
				jConfirm('<b>SAP System Message: </b><br>'+ spt[0], 'Sales Order Change', function(r) {
					if(r)
					{
						$('#loading').show();
						$("body").css("opacity","0.4");
						...
						$.ajax({
							...
							success: function(response)
							{
								...
								jAlert('<b>SAP System Message: </b><br> Changed Successfully', 'Sales Order Change');
							}
						});
					}
				});
				...
				jAlert('<b>SAP System Message: </b><br>'+ spt[0], 'Sales Order Change');
        }
    });
}
...
function strpdf()
{
	...
	$.ajax({
		type:'POST',
		data:'order_num='+number($('#I_VBELN').val())+'&bapi=editsalesorder_doc',
		url:'editsalesorder/stringpdf',
