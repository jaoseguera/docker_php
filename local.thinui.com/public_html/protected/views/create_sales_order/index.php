...
if(isset($_COOKIE['formdata']))
{
	...
	$ctrl_arr = ["SALES_ORG" => "sales_org", "PARTN_NUMB" => "cust", "DISTR_CHAN" => "distr_chan", "DOC_TYPE" => "sales_doc", "DIVISION" => "divi", "Delivery" => "dt"];
...
}elseif(isset($customerNo))
{
	...
	$material=ltrim($keys['MATERIAL'],"0");
      $arr_body[] = array('ITM_NUMBER' => $keys['ITM_NUMBER'], 'MATERIAL' =>$material  , 'SHORT_TEXT' => $keys['SHORT_TEXT'], 'REQ_QTY' => $keys['REQ_QTY'],'TARGET_QU' => $keys['TARGET_QU']);
	}
   if (isset($ORDER_HEADERS_OUT[0])) {
      ...
      $divi = $ORDER_HEADERS_OUT[0]['DIVISION'];
      $netvalue = $ORDER_HEADERS_OUT[0]['NET_VAL_HD'];
	  
			$sales_doc = '';
$dt=$ORDER_HEADERS_OUT[0]['REQ_DATE_H'];
...
<div id='loading'><img src="<?php echo Yii::app()->request->baseUrl; ?>/images/ajax-loader.gif"></div>
    <div class="utopia-widget-content myspace inval35 spaceing" style="margin-top:11px;">
        <form action="javascript:create_order();" method="post" id='validation' class="form-horizontal" enctype="multipart/form-data" autocomplete="on">
            <input type="hidden" name="url" value="create_sales_order"/>
            ...
            <div class="span5 utopia-form-freeSpace">
                <fieldset>
					...
					<div class="control-group">
                        <label class="control-label cutz" for="input01" id='PURCH_NO_C_s' alt='Purchase Order Number'><?php echo Controller::customize_label('Purchase Order Number');?><span> *</span>:</label>
                        <div class="controls">
							<input alt="Purchase Order Number" type="text" name='PURCH_NO_C' id='PURCH_NO_C' class="input-fluid validate[required] getval radius" value='<?php echo $PURCH_NO_C;?>' onKeyUp="jspt('PURCH_NO_C',this.value,event)" autocomplete="off"/>
						</div>
					</div>
                    <div class="control-group">
						<label class="control-label cutz" for="input01"  alt='Requested Delivery Date'><?php echo Controller::customize_label('Requested Delivery Date');?><span> *</span>:</label>
						<div class="controls">
							<input alt="Requested Delivery Date" type="text" name='Delivery' id='datepicker' value='<?php //echo $dt;?>' class="input-fluid validate[required,custom[date]] getval radius" />
						</div>
					</div>
                </fieldset>
            </div>
                                ...
                                <tbody>
									<?php if($dt == "" ) { ?>
										<tr onClick="select_row('ids_0')" class="ids_0 nudf" >
											...
											<td><img class="info-img" style="margin-left:-10%" src="<?php echo Yii::app()->request->baseUrl; ?>/images/icons/info.png"/><input type="text" id='MATERIAL' name='material[]' class="input-fluid validate[required] getval radiu " title="MATERIAL" alt="MULTI" onchange="jspt_new('MATERIAL',this.value,event)" onKeyUp="jspt('MATERIAL',this.value,event)" style="width: 55% !important" autocomplete="off" value='<?php echo $meterial;?>'/>
											...
											<td><input type="text" style="width:55% !important;" id='TARGET_QU' class="input-fluid validate[required] getval radiu" name='su[]' title="su" alt="MULTI"
									...
									<?php } if(is_array($arr_body)) 
									          { 
											  foreach($arr_body as $avk => $avv) { 
											  
											$itm_num	= $avv['ITM_NUMBER'];
											if(is_numeric($itm_num))
												$itm_num = ltrim($itm_num, "0");
											$meterial	= $avv['MATERIAL'];
											$dmeterial	= $avv['SHORT_TEXT'];
											$order_q	= $avv['REQ_QTY'];
											$su			= $avv['TARGET_QU'];
										?>
										<tr onClick="select_row('ids_<?php echo $avk; ?>')" class="ids_<?php echo $avk; ?> nudf" >
											<!--<td><input class="chkbox check" type="checkbox" name="checkbox[]" title="che" id="chedk"><input type="text" name='item[]' <?php if(!isset($customerNo)) { ?>value="000010"<?php } else {?>value=""<?php }?> style="width:90%;" title="item" class='input-fluid validate[required,custom[number]]' readonly alt="Items" id="ITM_NUMBER"/></td>-->
											<td><input class="chkbox check" type="checkbox" name="checkbox[]" title="che" id="chedk"><input type="text" name='item[]' value="<?php echo $itm_num;?>" style="width:90%;" title="item" class='input-fluid validate[required,custom[number]]' readonly alt="Items" id="ITM_NUMBER<?php echo $itm_num; ?>"/></td>
											<td><img class="info-img" style="margin-left:-10%" src="<?php echo Yii::app()->request->baseUrl; ?>/images/icons/info.png"/><input type="text" id='MATERIAL<?php echo $itm_num; ?>' name='material[]' class="input-fluid validate[required] getval radiu " title="MATERIAL" alt="MULTI" onchange="jspt_new('MATERIAL<?php echo $itm_num; ?>',this.value,event)" onKeyUp="jspt('MATERIAL<?php echo $itm_num; ?>',this.value,event)" autocomplete="off" style="width: 55% !important " value='<?php echo $meterial;?>'/>
											...
											<td><input type="text" id='SHORT_TEXT<?php echo $itm_num; ?>' style="width:90%;" class="input-fluid validate[required] getval" name='description[]' title="descript" alt="MULTI" onKeyUp="jspt('SHORT_TEXT<?php echo $itm_num; ?>',this.value,event)" autocomplete="off" value='<?php echo $dmeterial;?>'/></td>
											<td ><input type="text" id='REQ_QTY<?php echo $itm_num; ?>' style="width:90%;" class="input-fluid validate[required,custom[number]] getval" name='Order_quantity[]' alt="MULTI" onKeyUp="jspt('REQ_QTY<?php echo $itm_num; ?>',this.value,event)" autocomplete="off" value='<?php echo $order_q;?>'/></td>
											<td><input type="text" style="width:55% !important;" id='TARGET_QU<?php echo $itm_num; ?>' class="input-fluid validate[required] getval radiu" name='su[]' title="su" alt="MULTI"
											onKeyUp="jspt('TARGET_QU<?php echo $itm_num; ?>',this.value,event)" autocomplete="off" value='<?php echo $su;?>'/>
									...
									<?php }	} else { foreach($arr_val['ITM_NUMBER'] as $avk => $avv) { 
										...
										<tr onClick="select_row('ids_<?php echo $avk; ?>')" class="ids_<?php echo $avk; ?> nudf" >
											...
											<td><img class="info-img" style="margin-left:-10%" src="<?php echo Yii::app()->request->baseUrl; ?>/images/icons/info.png"/><input type="text" id='MATERIAL<?php echo $id_num; ?>' name='material[]' class="input-fluid validate[required] getval radiu " title="MATERIAL" alt="MULTI" onchange="jspt_new('MATERIAL<?php echo $id_num; ?>',this.value,event)" onKeyUp="jspt('MATERIAL<?php echo $id_num; ?>',this.value,event)" autocomplete="off" style="width: 55% !important" value='<?php echo $meterial;?>'/>
											<!--<span class='minw9' onclick="tipup('BUS2032','CREATEFROMDAT2','ORDERITEMSIN','MATERIAL','Material','MATERIAL<?php /*echo $id_num; */?>','3@MAT1L')" >&nbsp;</span>-->
                                                <div class='minws1' style="" onclick="lookup('Material', 'MATERIAL', 'material')" >&nbsp;</div>
											...
											<td><input type="text" style="width:55% !important;" id='TARGET_QU<?php echo $id_num; ?>' class="input-fluid validate[required] getval radiu" name='su[]' title="su" alt="MULTI"
...
function create_order()
{
    ...
    $.ajax({
        ...
        url: 'create_sales_order/order_sales',			
