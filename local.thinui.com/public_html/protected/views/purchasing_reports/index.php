<?php
	...
	$purchasing_reports = $doc->reports->$System_ID->$Client_ID->purchasing_reports;
        ...
        <section id='utopia-wizard-form' class="utopia-widget utopia-form-box" >
            <div class="row-fluid">
				<div class="contt span12">
					<ul class="ullink">
						<?php
							foreach($all_reports['purchasing'] as $key => $val)
							{
								if(isset($purchasing_reports))
								{
									if(isset($purchasing_reports->$key) && !empty($purchasing_reports->$key))
										echo "<li><a href='".$purchasing_reports->$key."' target='_blank' class='report_link' id='".$key."' onmousedown='mosds(event,this)'>".$val."</a></li>";
									else
	...
	jPrompturl('Please enter URL', editval,'Please enter URL',function(r){
		if(r)
		{
			...
			$.ajax({
				...
				url:'purchasing_reports/saveurl',
