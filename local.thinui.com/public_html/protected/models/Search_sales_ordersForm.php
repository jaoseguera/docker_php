...
class Search_sales_ordersForm extends CFormModel
{
    public $username;
    public $password;
    public $rememberMe;
    ...
    **/
     ...
     public function _actionSubmit($doc, $screen, $fce)
    {
		...
		$options = ['rtrim'=>true];
		try{
		$res = $fce->invoke(["CUSTOMER_NUMBER"=>$cust_num,
							"SALES_ORGANIZATION" =>$sale,
							"DOCUMENT_DATE"=>$date,
							"DOCUMENT_DATE_TO"=>$dateto,
							"TRANSACTION_GROUP"=>"0"],$options);								
		...
		$tableField1  = $screen.'_Search_sales_orders';
