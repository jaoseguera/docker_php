 ...
 * 
 */
function store_tableheader_couchdb(tableId)
{
	var titles = '';
	var titles_none = '';
	var let    = 0;
	var asw    = $('#'+tableId).parent('div').parent('div').closest('div').attr('id');			
	$("."+tableId+"_th").each(function()
	{
		// var fid = $(this).parent('th').closest('div').hasClass('head_fix');				
		// alert($(this).parent('th').css('display'));
		// console.log($(this).attr('name')+" "+$(this).parent('th').css('display'));
		...
		{
			titles_none += $(this).attr('name')+',';
		}
	})
	
	if(let==3)
	{
		var df=titles_none.split(',');
		titles += df[0]+','+df[1]+',';
	}
	if(let==1)
	{
		var df=titles_none.split(',');
		titles += df[0]+','+df[1]+','+df[2]+','+df[3]+',';
	}
	// url: "../lib/table_order.php",
	var urls= $('#'+tableId).attr('alt');
	//if($('#'+tableId).attr('alt')=='Sales_order_dashoard' && tableId=='example2')
	//{
	//	urls='delivery_list';
	//}
	$.ajax({
		type: "POST",
		url: $("#tableordersaveUrl").val(),
		data: "lables="+titles+"&url="+urls+"&sess="+asw, 
  ...
  $.widget("jb.dragtable", {
		...
		_mousemoveHandler: function( e ){
            ...
            .one( 'mouseup.' + self.widgetEventPrefix ,function(e ){
				//$('.head_fix').remove();
				//new FixedHeader( $('#'+o.tableId).dataTable());
				//	$('.head_fix').hide();
				var titles='';
				var titles_none='';
				var let=0;
				...
				//alert($('#'+o.tableId).attr('alt'));
				// url: "../lib/table_order.php",
				$.ajax({
					type: "POST",
					url: $("#tableordersaveUrl").val(),
					data: "lables="+titles+"&url="+$('#'+o.tableId).attr('alt')+"&sess="+asw,
					success: function(html) {
