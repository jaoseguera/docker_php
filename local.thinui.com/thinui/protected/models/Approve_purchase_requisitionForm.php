 ...
 **/
class Approve_purchase_requisitionForm extends CFormModel
{
    public $username;
    public $password;
    public $rememberMe;
    ...
    **/
    ...
    public function _actionSubmit($doc, $screen, $fce)
    {
		...
		if(count($exp)>0)
			$_SESSION['table_today1_count']=count($exp)-1;
		else
			$_SESSION['table_today1_count']=11;
		if(count($exp)<11)
		{
		...
		$_SESSION['table_today1'] = $REQUISITION_ITEMS;
	...
	public function _actionColumncount($doc, $screen)
    {
		...
		if(count($exp)>0)
			...
			$c_c=10;
		$_SESSION['table_today1_count']=$c_c; 	
		return $c_c;	
	}
}