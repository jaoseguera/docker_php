...
{    
	...
	if(type!='plant' && type!='uom' && type!='sold_to_customer' && type!='vendor' && type!='storage_loc' && type!='sales_document' && type!='material' && type!='sales_person')
	{
	values=$('#'+ctrl).val();
	if(ctrl=='PARTN_NUMB1')
...
function getLookUpResults(sh,type,para,ids,obj,method,sel,values){
   ...
   $(".fut").each(function(){
    ...
    if(values!='')        
        ...
        else if(type=='MATERIAL_PR'){
           var datastr = 'url=help&key=search_material_purch_req&val='+emails+'&em='+className+'&sh='+sh+'&type='+type+'&para='+para+'&ids='+ids+'&obj='+obj+'&method='+method+'&sel='+sel;
        }
