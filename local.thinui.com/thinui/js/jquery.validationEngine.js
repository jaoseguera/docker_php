 ...
 */
 (function($) {
	 ...
	 var methods = {
		...
		*/
		...
		$('.dynamicDiv').remove();
			...
			if(er==0)
			{
			 r=methods._validateFields(form, options.ajaxFormValidation);
	 ...
	 $("body").css("opacity","0.4"); 
	  $("body").css("filter","alpha(opacity=40)");
		...
		*/
		...
		/**
		* Field will not match default values 
		*
		* @param {jqObject} field
		* @param {Array[String]} rules
		* @param {int} i rules index
		...
		*/
		_notequals: function(field, rules, i, options) {
			var notequalsField = rules[i + 1];
			var fld=field.val().toLowerCase();
			var efld=$("#" + notequalsField).val().toLowerCase();
			if(efld=='emg')
				fld=fld.substring(0,3).toLowerCase();
			if ( fld==efld  )
		...
		*/
		_calculatePosition: function (field, promptElmt, options) {
			...
			return {
				...
				"callerleftPosition": (promptleftPosition-100) + "px",
	...
	$.validationEngine= {fieldIdCounter: 0,defaults:{
		...
		validationEventTrigger: "onSubmit",
